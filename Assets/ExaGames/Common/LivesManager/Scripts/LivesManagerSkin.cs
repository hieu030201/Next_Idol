...
namespace ExaGames.Common {
	...
	public class LivesManagerSkin : MonoBehaviour {
		#region Constants
	
		private const string MAX_LIVES_SAVEKEY_SKIN = "ExaGames.Common.MaxLivesSkin";
	
		private const string LIVES_SAVEKEY_SKIN = "ExaGames.Common.LivesSkin";
	
		private const string RECOVERY_TIME_SAVEKEY_SKIN = "ExaGames.Common.LivesRecoveryTimeSkin";

		private const string INFINITE_LIVES_TIME_SAVEKEY_SKIN = "ExaGames.Common.InfiniteLivesStartTimeSkin";

		private const string INFINITE_LIVES_MINUTES_SAVEKEY_SKIN = "ExaGames.Common.InfiniteLivesMinutesSkin";
		#endregion
		...
		public void ChangeRecoveryTime(double newSeconds)
		{
			if (newSeconds <= 0f)
			{
				return;
			}
			Debug.Log("ChangeRecoveryTime:" +newSeconds);
			// Nếu đang có infinite lives thì đếch cần làm gì
			//if (HasInfiniteLives) return;

			// Cập nhật MinutesToRecover thành phút (cho mấy hàm khác dùng)
			MinutesToRecover = newSeconds / 60f;
			
			secondsToNextLife = newSeconds;
		...
		private void retrievePlayerPrefs() {
			...
			MaxLives = PlayerPrefs.HasKey(MAX_LIVES_SAVEKEY_SKIN) ? PlayerPrefs.GetInt(MAX_LIVES_SAVEKEY_SKIN) : DefaultMaxLives;
			if(PlayerPrefs.HasKey(INFINITE_LIVES_TIME_SAVEKEY_SKIN) && PlayerPrefs.HasKey(INFINITE_LIVES_MINUTES_SAVEKEY_SKIN)) {
				infiniteLivesStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(INFINITE_LIVES_TIME_SAVEKEY_SKIN)));
				infiniteLivesMinutes = PlayerPrefs.GetInt(INFINITE_LIVES_MINUTES_SAVEKEY_SKIN);
			} else {
				infiniteLivesStartTime = DateTime.MinValue;
				infiniteLivesMinutes = 0;
			}
			if(PlayerPrefs.HasKey(LIVES_SAVEKEY_SKIN) && PlayerPrefs.HasKey(RECOVERY_TIME_SAVEKEY_SKIN)) {
				lives = PlayerPrefs.GetInt(LIVES_SAVEKEY_SKIN);
				recoveryStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(RECOVERY_TIME_SAVEKEY_SKIN)));
			} else {
				lives = MaxLives;
				recoveryStartTime = DateTime.Now;
			}
		...
		private void savePlayerPrefs() {
			PlayerPrefs.SetInt(MAX_LIVES_SAVEKEY_SKIN, MaxLives);
			PlayerPrefs.SetInt(LIVES_SAVEKEY_SKIN, lives);
			PlayerPrefs.SetString(RECOVERY_TIME_SAVEKEY_SKIN, recoveryStartTime.Ticks.ToString());
			PlayerPrefs.SetString(INFINITE_LIVES_TIME_SAVEKEY_SKIN, infiniteLivesStartTime.Ticks.ToString());
			PlayerPrefs.SetInt(INFINITE_LIVES_MINUTES_SAVEKEY_SKIN, infiniteLivesMinutes);
			try {
				PlayerPrefs.Save();
			} catch(Exception e) {
		...
		public void ResetPlayerPrefs() {
			PlayerPrefs.DeleteKey(MAX_LIVES_SAVEKEY_SKIN);
			PlayerPrefs.DeleteKey(RECOVERY_TIME_SAVEKEY_SKIN);
			PlayerPrefs.DeleteKey(LIVES_SAVEKEY_SKIN);
			PlayerPrefs.DeleteKey(INFINITE_LIVES_TIME_SAVEKEY_SKIN);
			PlayerPrefs.DeleteKey(INFINITE_LIVES_MINUTES_SAVEKEY_SKIN);
			clearNotification();
			retrievePlayerPrefs();
		}
		#endregion
