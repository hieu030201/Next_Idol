...
namespace ExaGames.Common {
	...
	public class LivesManagerShopThird : MonoBehaviour {
		#region Constants
	
		private const string MAX_LIVES_SAVEKEY_SHOP_THIRD = "ExaGames.Common.MaxLivesShopThird";
	
		private const string LIVES_SAVEKEY_SHOP_THIRD = "ExaGames.Common.LivesShopThird";
	
		private const string RECOVERY_TIME_SAVEKEY_SHOP_THIRD = "ExaGames.Common.LivesRecoveryTimeShopThird";

		private const string INFINITE_LIVES_TIME_SAVEKEY_SHOP_THIRD = "ExaGames.Common.InfiniteLivesStartTimeShopThird";

		private const string INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_THIRD = "ExaGames.Common.InfiniteLivesMinutesShopThird";
		#endregion
		...
		private void retrievePlayerPrefs() {
			...
			MaxLives = PlayerPrefs.HasKey(MAX_LIVES_SAVEKEY_SHOP_THIRD) ? PlayerPrefs.GetInt(MAX_LIVES_SAVEKEY_SHOP_THIRD) : DefaultMaxLives;
			if(PlayerPrefs.HasKey(INFINITE_LIVES_TIME_SAVEKEY_SHOP_THIRD) && PlayerPrefs.HasKey(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_THIRD)) {
				infiniteLivesStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(INFINITE_LIVES_TIME_SAVEKEY_SHOP_THIRD)));
				infiniteLivesMinutes = PlayerPrefs.GetInt(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_THIRD);
			} else {
				infiniteLivesStartTime = DateTime.MinValue;
				infiniteLivesMinutes = 0;
			}
			if(PlayerPrefs.HasKey(LIVES_SAVEKEY_SHOP_THIRD) && PlayerPrefs.HasKey(RECOVERY_TIME_SAVEKEY_SHOP_THIRD)) {
				lives = PlayerPrefs.GetInt(LIVES_SAVEKEY_SHOP_THIRD);
				recoveryStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(RECOVERY_TIME_SAVEKEY_SHOP_THIRD)));
			} else {
				lives = MaxLives;
				recoveryStartTime = DateTime.Now;
			}
		...
		private void savePlayerPrefs() {
			PlayerPrefs.SetInt(MAX_LIVES_SAVEKEY_SHOP_THIRD, MaxLives);
			PlayerPrefs.SetInt(LIVES_SAVEKEY_SHOP_THIRD, lives);
			PlayerPrefs.SetString(RECOVERY_TIME_SAVEKEY_SHOP_THIRD, recoveryStartTime.Ticks.ToString());
			PlayerPrefs.SetString(INFINITE_LIVES_TIME_SAVEKEY_SHOP_THIRD, infiniteLivesStartTime.Ticks.ToString());
			PlayerPrefs.SetInt(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_THIRD, infiniteLivesMinutes);
			try {
				PlayerPrefs.Save();
			} catch(Exception e) {
		...
		public void ResetPlayerPrefs() {
			PlayerPrefs.DeleteKey(MAX_LIVES_SAVEKEY_SHOP_THIRD);
			PlayerPrefs.DeleteKey(RECOVERY_TIME_SAVEKEY_SHOP_THIRD);
			PlayerPrefs.DeleteKey(LIVES_SAVEKEY_SHOP_THIRD);
			PlayerPrefs.DeleteKey(INFINITE_LIVES_TIME_SAVEKEY_SHOP_THIRD);
			PlayerPrefs.DeleteKey(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_THIRD);
			clearNotification();
			retrievePlayerPrefs();
		}
		#endregion
