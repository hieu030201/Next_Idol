...
namespace ExaGames.Common {
	...
	public class LivesManagerShopSecond : MonoBehaviour {
		#region Constants
	
		private const string MAX_LIVES_SAVEKEY_SHOP_SECOND = "ExaGames.Common.MaxLivesShopSecond";
	
		private const string LIVES_SAVEKEY_SHOP_SECOND = "ExaGames.Common.LivesShopSecond";
	
		private const string RECOVERY_TIME_SAVEKEY_SHOP_SECOND = "ExaGames.Common.LivesRecoveryTimeShopSecond";

		private const string INFINITE_LIVES_TIME_SAVEKEY_SHOP_SECOND = "ExaGames.Common.InfiniteLivesStartTimeShopSecond";

		private const string INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_SECOND = "ExaGames.Common.InfiniteLivesMinutesShopSecond";
		#endregion
		...
		public void AddLifeSlots(int quantity, bool fillLives = false) {
			if(HasMaxLives && !HasInfiniteLives) {
				...
				Debug.Log("Run in AddLifeSlots Second");
			}
			MaxLives += quantity;
			if(fillLives) {
				FillLives();
		...
		private void retrievePlayerPrefs() {
			...
			MaxLives = PlayerPrefs.HasKey(MAX_LIVES_SAVEKEY_SHOP_SECOND) ? PlayerPrefs.GetInt(MAX_LIVES_SAVEKEY_SHOP_SECOND) : DefaultMaxLives;
			if(PlayerPrefs.HasKey(INFINITE_LIVES_TIME_SAVEKEY_SHOP_SECOND) && PlayerPrefs.HasKey(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_SECOND)) {
				infiniteLivesStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(INFINITE_LIVES_TIME_SAVEKEY_SHOP_SECOND)));
				infiniteLivesMinutes = PlayerPrefs.GetInt(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_SECOND);
			} else {
				infiniteLivesStartTime = DateTime.MinValue;
				infiniteLivesMinutes = 0;
			}
			if(PlayerPrefs.HasKey(LIVES_SAVEKEY_SHOP_SECOND) && PlayerPrefs.HasKey(RECOVERY_TIME_SAVEKEY_SHOP_SECOND)) {
				lives = PlayerPrefs.GetInt(LIVES_SAVEKEY_SHOP_SECOND);
				recoveryStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(RECOVERY_TIME_SAVEKEY_SHOP_SECOND)));
			} else {
				lives = MaxLives;
				recoveryStartTime = DateTime.Now;
			}
		...
		private void savePlayerPrefs() {
			PlayerPrefs.SetInt(MAX_LIVES_SAVEKEY_SHOP_SECOND, MaxLives);
			PlayerPrefs.SetInt(LIVES_SAVEKEY_SHOP_SECOND, lives);
			PlayerPrefs.SetString(RECOVERY_TIME_SAVEKEY_SHOP_SECOND, recoveryStartTime.Ticks.ToString());
			PlayerPrefs.SetString(INFINITE_LIVES_TIME_SAVEKEY_SHOP_SECOND, infiniteLivesStartTime.Ticks.ToString());
			PlayerPrefs.SetInt(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_SECOND, infiniteLivesMinutes);
			try {
				PlayerPrefs.Save();
			} catch(Exception e) {
		...
		public void ResetPlayerPrefs() {
			PlayerPrefs.DeleteKey(MAX_LIVES_SAVEKEY_SHOP_SECOND);
			PlayerPrefs.DeleteKey(RECOVERY_TIME_SAVEKEY_SHOP_SECOND);
			PlayerPrefs.DeleteKey(LIVES_SAVEKEY_SHOP_SECOND);
			PlayerPrefs.DeleteKey(INFINITE_LIVES_TIME_SAVEKEY_SHOP_SECOND);
			PlayerPrefs.DeleteKey(INFINITE_LIVES_MINUTES_SAVEKEY_SHOP_SECOND);
			clearNotification();
			retrievePlayerPrefs();
		}
		#endregion
