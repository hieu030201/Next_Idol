...
namespace ExaGames.Common {
	...
	public class LivesManagerSkinItem : MonoBehaviour {
		#region Constants

		private const string MAX_LIVES_SAVEKEY_FORMAT = "ExaGames.Common.MaxLivesSkinItem.{0}";
	
		private const string LIVES_SAVEKEY_FORMAT = "ExaGames.Common.LivesSkinItem.{0}";
	
		private const string RECOVERY_TIME_SAVEKEY_FORMAT = "ExaGames.Common.LivesRecoveryTimeSkinItem.{0}";

		private const string INFINITE_LIVES_TIME_SAVEKEY_FORMAT = "ExaGames.Common.InfiniteLivesStartTimeSkinItem.{0}";

		private const string INFINITE_LIVES_MINUTES_SAVEKEY_FORMAT = "ExaGames.Common.InfiniteLivesMinutesSkinItem.{0}";
		#endregion

		#region Fields
		[SerializeField] public string itemId = "default"; 
		...
		{
			return HasInfiniteLives ? remainingSecondsWithInfiniteLives : secondsToNextLife;
		}

		#region Data persistance
		private string GetMaxLivesKey() => string.Format(MAX_LIVES_SAVEKEY_FORMAT, itemId);
		private string GetLivesKey() => string.Format(LIVES_SAVEKEY_FORMAT, itemId);
		private string GetRecoveryTimeKey() => string.Format(RECOVERY_TIME_SAVEKEY_FORMAT, itemId);
		private string GetInfiniteLivesTimeKey() => string.Format(INFINITE_LIVES_TIME_SAVEKEY_FORMAT, itemId);
		private string GetInfiniteLivesMinutesKey() => string.Format(INFINITE_LIVES_MINUTES_SAVEKEY_FORMAT, itemId);
		  private void retrievePlayerPrefs() {
		        remainingSecondsWithInfiniteLives = 0D;
		        MaxLives = PlayerPrefs.HasKey(GetMaxLivesKey()) ? PlayerPrefs.GetInt(GetMaxLivesKey()) : DefaultMaxLives;
		        
		        if(PlayerPrefs.HasKey(GetInfiniteLivesTimeKey()) && PlayerPrefs.HasKey(GetInfiniteLivesMinutesKey())) {
		            infiniteLivesStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(GetInfiniteLivesTimeKey())));
		            infiniteLivesMinutes = PlayerPrefs.GetInt(GetInfiniteLivesMinutesKey());
		        } else {
		            infiniteLivesStartTime = DateTime.MinValue;
		            infiniteLivesMinutes = 0;
		        }
		        
		        if(PlayerPrefs.HasKey(GetLivesKey()) && PlayerPrefs.HasKey(GetRecoveryTimeKey())) {
		            lives = PlayerPrefs.GetInt(GetLivesKey());
		            recoveryStartTime = new DateTime(long.Parse(PlayerPrefs.GetString(GetRecoveryTimeKey())));
		        } else {
		            lives = MaxLives;
		            recoveryStartTime = DateTime.Now;
		        }
		        ...
		        if (PlayerPrefs.HasKey("LastSavedTime_" + itemId)) {
			        DateTime lastSavedTime = new DateTime(long.Parse(PlayerPrefs.GetString("LastSavedTime_" + itemId)));
			        elapsedSeconds = (DateTime.Now - lastSavedTime).TotalSeconds;
		        }
		        
		        if (PlayerPrefs.HasKey("RemainingTime_" + itemId)) {
			        secondsToNextLife = PlayerPrefs.GetFloat("RemainingTime_" + itemId);
		        } else {
			        secondsToNextLife = MinutesToRecover * 60;
		        }

		        if (PlayerPrefs.HasKey("RemainingInfiniteTime_" + itemId)) {
			        remainingSecondsWithInfiniteLives = PlayerPrefs.GetFloat("RemainingInfiniteTime_" + itemId);
		        }
		    ...
		    private void savePlayerPrefs() {
		        PlayerPrefs.SetInt(GetMaxLivesKey(), MaxLives);
		        PlayerPrefs.SetInt(GetLivesKey(), lives);
		        PlayerPrefs.SetString(GetRecoveryTimeKey(), recoveryStartTime.Ticks.ToString());
		        PlayerPrefs.SetString(GetInfiniteLivesTimeKey(), infiniteLivesStartTime.Ticks.ToString());
		        PlayerPrefs.SetInt(GetInfiniteLivesMinutesKey(), infiniteLivesMinutes);
		        
		        PlayerPrefs.SetFloat("RemainingTime_" + itemId, (float)secondsToNextLife);
		        PlayerPrefs.SetFloat("RemainingInfiniteTime_" + itemId, (float)remainingSecondsWithInfiniteLives);

		        // Lưu thời điểm OnDestroy được gọi
		        PlayerPrefs.SetString("LastSavedTime_" + itemId, DateTime.Now.Ticks.ToString());
		        try {
		            PlayerPrefs.Save();
		        } catch(Exception e) {
		            Debug.LogError($"Could not save LivesManager preferences for item {itemId}: " + e.Message);
		        }
		    }

		    public void ResetPlayerPrefs() {
		        PlayerPrefs.DeleteKey(GetMaxLivesKey());
		        PlayerPrefs.DeleteKey(GetRecoveryTimeKey());
		        PlayerPrefs.DeleteKey(GetLivesKey());
		        PlayerPrefs.DeleteKey(GetInfiniteLivesTimeKey());
		        PlayerPrefs.DeleteKey(GetInfiniteLivesMinutesKey());
		        clearNotification();
		        retrievePlayerPrefs();
		    }
