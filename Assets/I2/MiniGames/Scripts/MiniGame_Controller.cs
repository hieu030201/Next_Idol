...
[System.Serializable]
public class UnityEventCurrencyCallbk : UnityEvent<int, System.Action<bool>>
{
...
public class MiniGame_Controller : MonoBehaviour
{
    ...
    public float
        _TimeBeforeStartRound = 0; // This delay can be used to start the round after some effects/sounds are finished

    //--[ Events to hide/show labels and update texts ]--------------

    public UnityEvent
        _OnStartPlaying =
            new UnityEvent(); // Show/Hide objects for the initial state, execute the entrance effect and call OnReadyForNextRound right after it finishes

    public UnityEvent _OnRoundStarted = new UnityEvent();
    public UnityEvent _OnGameOver = new UnityEvent();
    public Action<int> SetUpController;
    public Action<int> CalculateFree;
    public Action<int> CalculateTicket;
    public Action CalculateAds;

    //--[ Free Rounds ]---------------------
    [Header("Free Rounds")] public int _InitialFreeRounds; // How many rounds are allowed without having to pay for them

    [Multiline]
    public string
        _FreeRoundsLabelFormat =
            "{0} left"; // The status text will show the num free rounds using this format {0} is replaced by the remaining rounds

    public UnityEventString
        _OnUpdateFreeRounds =
            new UnityEventString(); // Shows the Free Round button and updates the label's text following the _FreeRoundsLabelFormat

    //--[ Purchaseable Rounds ]------------------

    [Header("Purchasable Rounds")] [Multiline]
    public string _CostLabelFormat = "{0} coins for another chance";
    ...
    public UnityEventString
        _OnUpdateRoundCost = new UnityEventString(); // Hides the Free Round button, shows the Buy Chance

    //--[ Currency Callbacks ]----------

    [Header("Currency Management")]
    public UnityEvent
        _OnShouldBuyCurrency =
            new UnityEvent(); // Open the popup to buy currency and when finished calls ValidateRound again or DenyRound

    public UnityEventCurrencyCallbk
        _OnConsumeCurrency =
            new UnityEventCurrencyCallbk(); // Consume currency and returns the result using the supplied callback
    ...
    public void StartGame()
    {
        ...
        else if (_TimeBeforeStartRound >= 0)
            OnReadyForNextRound();
    }
    ...
    public void BtnFreeClick(int amount)
    {
        mNumFreeRounds = amount;
        mIsPlaying = true;
        ValidateRound();
        CalculateFree.Invoke(-1);
    }

    public void BtnTicketClick()
    {
        mIsPlaying = true;
        AddCurrency(1);
        ValidateRound();

        CalculateTicket.Invoke(-1);
    }
