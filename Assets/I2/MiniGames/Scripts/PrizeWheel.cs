...
namespace I2.MiniGames
{
	...
	public class PrizeWheel : MiniGame 
	{
		...
		public override void StartRound()
		{
			...
			xWheel = cameraRotation.eulerAngles.x;
			yWheel = cameraRotation.eulerAngles.y;
			Debug.Log("xWheel: " + xWheel + " yWheel: " + yWheel);
			...
			for (int i=0, imax=mRewards.Count; i<imax; ++i)
				{
					...
					{
						Angle += (mWheelMaxAngleForKnot +3);  // Don't allow too close to the begining in case there is a knot there
						if (Angle >= 0)
						{
		...
		private bool UpdateRotation( float initialAngle, float totalRotation, float initialTime, float totalTime, float speedUpTime)
		{
			...
			{
				_Wheel.rotation = Quaternion.Euler(xWheel, yWheel, angle);
				_Selector.rotation = Quaternion.Euler(xWheel, yWheel, angle);
			}
		...
		private bool UpdateSelector( ref int currentKnot, bool rotateback )
		{
			...
			_Selector.rotation = Quaternion.Euler(xWheel,yWheel, currentSelectorAngle);
			return finished;
		}
